--------------------------------------------------------
--  DDL for Procedure UPDATE_TOTAL_TAX_SET
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "UPDATE_TOTAL_TAX_SET" 
(
  P_ID IN NUMBER,
  P_BODY_TEXT IN CLOB 
) AS 
BEGIN
UPDATE sp_total_tax_set ts
...
= (SELECT *
  FROM
    json_table(p_body_text, '$'
        ...
        ) jt WHERE ts.order_id = p_id
    );
END UPDATE_TOTAL_TAX_SET;
