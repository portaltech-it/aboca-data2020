--------------------------------------------------------
--  DDL for Procedure INSERT_TOTAL_SHIPPING_PRICE_SET
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MW_ECOMMERCE"."INSERT_TOTAL_SHIPPING_PRICE_SET" 
(
  P_BODY_TEXT IN CLOB 
) AS 
BEGIN
INSERT INTO sp_total_shipping_price_set
...
SELECT *
  FROM
    json_table(p_body_text, '$'
        columns(
            "id" NUMBER PATH '$.id',
            nested PATH '$.total_shipping_price_set[*]'
                columns(
...
END INSERT_TOTAL_SHIPPING_PRICE_SET;
