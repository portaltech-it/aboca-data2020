...
set define off;
  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "MW_ECOMMERCE"."INSERT_ORDER" (p_body_text IN CLOB)
AS 
BEGIN
...
(id, email, closed_at, created_at, updated_at, "NUMBER", note, token, gateway, test, total_price, subtotal_price, total_weight, total_tax, taxes_included, currency, 
    ...
    presentment_currency, total_tip_received, original_total_duties_set, current_total_duties_set, admin_graphql_api_id, payment_gateway_names) 
SELECT *
  FROM
